...
public class Lambda05 {
    ...
    public static int toplaCincix(int x) {
       return IntStream.range(0,x+1).//1 den x'e kadar(x dahil x+1 hariç) int degerler akısa alındı
    ...
    public static int toplaCift(int x) {
     return  IntStream.rangeClosed(1,x).//1 den x'e kadar(x dahil) int degerler akısa alındı
             filter(Lambda01::ciftBul).//method reference ıle çift sayılar fıltrelendı
             sum();//akıştan gelen sayılar toplandı
    }

    //TASK 03 --> Ilk x pozitif cift sayiyi toplayan program  create ediniz.

    public static int toplaIlkXCift(int x) {
      // return IntStream.rangeClosed(2,2*x).filter(Lambda01::ciftBul).sum();--> GÜLŞEN GENÇ LOGİC İ İLE :D
        return IntStream.
    ...
    public static void ikininIlkXKuvvetPrint(int x) {
        IntStream.iterate(2,t-> t*2).limit(x).forEach(Lambda01::yazdır);
    }
    ...
    public static void isteneSayiIlkXKuvvetPrint(int isteneSayi, int x) {
        IntStream.
                ...
                limit(x).//akısdaki ilk x int degeri akısa alır
                forEach(Lambda01::yazdır);//akısdaki int degerler toplandı
    }
    ...
    public static int xKuvveti(int istenenSayi, int x) {
      // return Math.pow(istenenSayi,x);--> Lambdasız :D ama double verıyor ,DİKKAT!
        return IntStream.iterate(istenenSayi,t-> t*istenenSayi).limit(x).skip(x-1).reduce(0,(t,u)-> u);
    }
